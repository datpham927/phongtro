...
const DetailPost: React.FC = () => {
    ...
    const breadcrumbs = [
        { path: `/${category.slug}`, breadcrumb: category.name },
        { path: `/tinh-thanh/${address.city_slug}`, breadcrumb: address.city_name },
        { path: `/tinh-thanh/${address.city_slug}/${address.district_slug}`, breadcrumb: address.district_name },
                ...
                <div className="w-2/3 overflow-hidden h-full gap-4 rounded-md">
                    <div className="w-full shadow-custom">
                        ...
                        <div className="p-5 bg-white">
                            ...
                            <div>
                                <span
                                    className="flex items-center w-fit text-center uppercase px-3 py-1 text-[10px] text-white rounded-sm"
                                    style={{ backgroundColor: POST_TYPE_COLOR[post_type.priority - 1] }}
                                >
                                    {[1, 2].includes(post_type.priority) && (
                                ...
                                <table className="w-full my-3">
                                    <tbody>
                                        ...
                                        {renderRow(
                                            'Chuyên mục:',
                                            <Link
                                                className="text-[#1266dd] underline hover:text-[#f60]"
                                                to={`/${category.slug}/${address.city_slug}/${address.district_slug}`}
                                            >
